...
class CommandParser:
    ...
    def execute(self, commands_file):
        ...
        waiting_period = Settings.waiting_period
        for leave_type in waiting_period:
            waiting_period[leave_type] += Settings.max_weeks[leave_type]
    ...
    def get_commands(commands_file):
        ...
        try:
            ...
            Settings.log_error('Commands file was  not found')
        ...
        for command in commands:
            ...
            if cmd_type in commands_dict:
                Settings.log_error('Command Error: ' + cmd_type.upper() + ' appears multiple times')
    ...
    def parse_commands(self, commands):
        try:
            ...
            Settings.log_error('Command Error: ' + missing_command + ' is required but was not supplied')
        ...
        if Settings.extend_leaves and not Settings.extend_old:
            try:
                ...
                Settings.log_error('EXTENDLEAVES is "Yes" and EXTENDOLD is "No" or not supplied, so '
    ...
    @staticmethod
    def parse_cmd_options(options, cmd_type):
        opts_dict = {}
        while len(options) > 0:
            ...
            if option not in opts_dict:
                ...
                Settings.log_error('The field "' + option + '" appears multiple times in command ' + cmd_type)

        return opts_dict

    @staticmethod
    def parse_boolean(commands, cmd_type):
        if commands[cmd_type].lower() == 'yes':
            return True
        elif commands[cmd_type].lower() == 'no':
            return False
        else:
            Settings.log_error('Command Error: ' + cmd_type.upper() + ' should be "Yes" or "No"')
    ...
    def process_file_cmd(self, options):
        ...
        try:
            ...
            Settings.log_error('Command Error: ' + missing_field + ' is required but missing from FILE command')
    ...
    def process_clone_factor(self, value):
        ...
        if value == "Error" or value < 1:
            Settings.log_error('Command Error: CLONEFACTOR should be a positive integer')
        return value

    def process_dependent_allowance(self, value):
        value = self.parse_float(value)
        if value == "Error" or value < 0:
            Settings.log_error('Command Error: DEPENDENTALLOWANCE should be a positive real number')
        return value

    def process_detail(self, value):
        value = self.parse_integer(value)
        if value == "Error" or value < 1 or value > 8:
            Settings.log_error('Command Error: DETAIL should be a positive integer between 1 and 8')
        return value
    ...
    def process_eligibility_rules(self, options):
        ...
        if 'type' in options:
            ...
            if el_type == 'fmla':
                ...
                Settings.log_error('Command Error: Must provide either FMLA or MA_UIB type in ELIGIBILITYRULES')
            return
    ...
    def process_extend_days(self, options):
        ...
        extend_days = Settings.extend_days
        for option in options:
            value = self.parse_integer(options[option])
            if value == 'Error' or value < 0:
                Settings.log_error('Command Error: The field values in EXTENDDAYS should be non-negative integers')
    ...
    def process_extend_proportions(self, options):
        ...
        extend_proportions = Settings.extend_proportions
        for option in options:
            value = self.parse_float(options[option])
            if value == 'Error' or value < 0:
                Settings.log_error('Command Error: The field values in EXTENDPROPORTIONS '
    ...
    def process_extend_probabilities(self, options):
        ...
        extend_probabilities = Settings.extend_probabilities
        for option in options:
            value = self.parse_integer(options[option])
            if value == 'Error':
                Settings.log_error('Command Error: The field values in EXTENDPROB should be real numbers')
            options[option] = value
    ...
    def process_formula2(self, options):
        ...
        if Settings.formula:
            Settings.log_error('Command Error: FORMULA2 command cannot be present if FORMULA command is "Yes"')

        Settings.formula_2 = True
        ...
        while len(options) > 0:
            ...
            value = self.parse_float(value)
            if value == 'Error':
                Settings.log_error('Command Error: FORMULA2 field values should be real numbers')
            ...
            if key == 'top':
                ...
                Settings.log_error('Command Error: ' + key.upper() + ' is not a valid field in FORMULA2 command')

        if len(Settings.f_cutoffs) < 1:
            Settings.log_error('Command Error: Must be at least one TOP field in the FORMULA2 command')

        if len(Settings.f_rates) != len(Settings.f_cutoffs) + 1:
            Settings.log_error('Command Error: Must be one more RATE field than TOP fields in FORMULA2 command')

        if not self.is_sorted(Settings.f_rates):
            Settings.log_error('Command Error: TOP fields in FORMULA2 command must be in increasing order')

    def process_leave_probability_factors(self, options):
        options = self.parse_cmd_options(options, 'LEAVEPROBABILITYFACTORS')
        leave_probability_factors = Settings.leave_probability_factors
        for option in options:
            value = self.parse_float(options[option])
            if value == 'Error':
                Settings.log_error('Command Error: The field values in LEAVEPROBABILITYFACTORS should be real numbers')
            options[option] = value
    ...
    def process_max_weeks(self, options):
        ...
        for option in options:
            value = self.parse_integer(options[option])
            if value == 'Error' or value < 1:
                Settings.log_error('Command Error: The field values in MAXWEEKS should be positive integers')
            options[option] = value
    ...
    def process_replacement_ratio(self, value):
        value = self.parse_float(value)
        if value == 'Error':
            Settings.log_error('Command Error: REPLACEMENTRATIO should be a real number')
        Settings.rep_ratio = value

    def process_state_of_work(self, value):
        value = self.parse_integer(value)
        if value == 'Error' or value < 1:
            Settings.log_error('Command Error: STATEOFWORK should be a positive integer')
        Settings.stateofwork = value

    def process_take_up_rates(self, options):
        options = self.parse_cmd_options(options, 'TAKEUPRATES')
        take_up_rates = Settings.take_up_rates
        for option in options:
            value = self.parse_float(options[option])
            if value == 'Error':
                Settings.log_error('Command Error: The field values in TAKEUPRATES should be real numbers')
            options[option] = value
    ...
    def process_top_off_min_length(self, value):
        value = self.parse_integer(value)
        if value == 'Error':
            Settings.log_error('Command Error: TOPOFFMINLENGTH should be an integer')
        Settings.topoffminlength = value

    def process_top_rate1(self, value):
        value = self.parse_float(value)
        if value == 'Error':
            Settings.log_error('Command Error: TOPOFFRATE1 should be a real number')
        Settings.topoffrate1 = value
    ...
    def process_waiting_period(self, options):
        ...
        for option in options:
            value = self.parse_integer(options[option])
            if value == 'Error' or value < 0:
                Settings.log_error('Command Error: The field values in WAITINGPERIOD should be non-negative integers')
            options[option] = value
    ...
    def process_weekly_ben_cap(self, value):
        value = self.parse_float(value)
        if value == 'Error':
            Settings.log_error('Command Error: WEEKLYBENCAP should be a real number')
        Settings.cap = value

    def process_weight_factor(self, value):
        value = self.parse_float(value)
        if value == 'Error':
            Settings.log_error('Command Error: WEIGHTFACTOR should be a real number')
        Settings.n_years = value
